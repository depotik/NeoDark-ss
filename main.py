...
def execute_script(script_number):
    ...
    if script_path.exists():
        try:
            ...
            print_centered(f"{Colors.RED}❌ Ошибка при выполнении скрипта {filename}{Colors.RESET}")
            print_centered(f"{Colors.YELLOW}💡 Код ошибки: {e.returncode}{Colors.RESET}")
...
def show_startup_warning():
    ...
    # Ждем 8 секунды перед добавлением серой надписи
    import time
    time.sleep(8)
    ...
    # Ждем еще 2 секунды (всего 10 секунд с начала отображения)
    time.sleep(2)
